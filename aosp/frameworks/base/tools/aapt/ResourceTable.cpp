...
int ResourceTable::getPublicAttributeSdkLevel(uint32_t attrId) const {
    ...
    if (entryId <= 0x021c) {
        ...
        return SDK_LOLLIPOP;
    } else if (entryId <= 0x04d8) {
        return SDK_LOLLIPOP_MR1;
    } else if (entryId <= 0x04f1) {
        return SDK_MARSHMALLOW;
    } else if (entryId <= 0x0527) {
        return SDK_NOUGAT;
    } else if (entryId <= 0x0530) {
        return SDK_NOUGAT_MR1;
    } else if (entryId <= 0x0568) {
        return SDK_O;
    } else if (entryId <= 0x056d) {
        return SDK_O_MR1;
    } else if (entryId <= 0x0586) {
        return SDK_P;
    } else if (entryId <= 0x0606) {
        return SDK_Q;
    } else {
        return SDK_R;
    }
